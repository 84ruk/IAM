...
export class TwoFactorController {
  ...
  async setupTwoFactor(
    ...
    try {
      ...
      if (userId !== user.id && user.rol !== 'SUPERADMIN') {
        throw new BadRequestException('No tienes permisos para configurar 2FA para otros usuarios');
      }
  ...
  async verifyTwoFactor(
    ...
    try {
      ...
      if (userId !== user.id && user.rol !== 'SUPERADMIN') {
        throw new BadRequestException('No tienes permisos para verificar 2FA para otros usuarios');
      }

      const result = await this.twoFactorService.verifyTwoFactor(userId, dto.token);
  ...
  async enableTwoFactor(
    ...
    try {
      ...
      const verification = await this.twoFactorService.verifyTwoFactor(user.id, dto.token);
      ...
      return { message: 'Autenticación de dos factores habilitada exitosamente' };
    } catch (error) {
      throw new BadRequestException(error.message);
    }
  }
  ...
  @ApiResponse({ status: 400, description: 'Código inválido o confirmación requerida' })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  async disableTwoFactor(
    ...
    try {
      if (!dto.confirm) {
        throw new BadRequestException('Debes confirmar la deshabilitación de 2FA');
      }
  ...
  @Get('status/:userId')
  @ApiOperation({ summary: 'Obtener estado de 2FA de otro usuario (solo SUPERADMIN)' })
  @ApiResponse({ 
    status: 200, 
  ...
  async getTwoFactorStatusForUser(
    ...
    try {
      ...
      if (user.rol !== 'SUPERADMIN') {
        throw new BadRequestException('No tienes permisos para ver el estado de 2FA de otros usuarios');
      }
