...
export interface SecurityEvent {
  type: 'login_attempt' | 'failed_login' | 'suspicious_activity' | 'rate_limit_exceeded' | 'token_blacklisted' | '2fa_failed' | 'admin_action';
  userId?: number;
  userEmail?: string;
  ipAddress: string;
  userAgent: string;
...
export class SecurityMonitoringService {
   ...
   */
  private async analyzeEvent(event: SecurityEvent): Promise<SecurityAlert | null> {
    ...
    switch (event.type) {
      case 'failed_login':
        const failedLogins = await this.getFailedLoginsCount(event.ipAddress, 15); // 15 minutos
        if (failedLogins >= 5) {
          action = 'block_ip';
          shouldAlert = true;
      ...
      case '2fa_failed':
        const twoFactorFailures = await this.getTwoFactorFailuresCount(event.userId || 0, 60); // 1 hora
  ...
  private async executeAlert(alert: SecurityAlert): Promise<void> {
    try {
      switch (alert.action) {
        case 'block_ip':
          await this.blockIP(alert.event.ipAddress, 'Security alert: ' + alert.event.type);
          break;
        ...
        case 'lock_account':
          if (alert.event.userId) {
            await this.lockUserAccount(alert.event.userId, 'Multiple 2FA failures');
          }
          break;
      }
  ...
  private async lockUserAccount(userId: number, reason: string): Promise<void> {
    ...
    this.secureLogger.logSecurityError(`User account locked: ${userId} - ${reason}`, userId);
  }
   ...
   */
  private async getFailedLoginsCount(ipAddress: string, minutes: number): Promise<number> {
   ...
   */
  private async getTwoFactorFailuresCount(userId: number, minutes: number): Promise<number> {
    const since = new Date(Date.now() - minutes * 60 * 1000);
