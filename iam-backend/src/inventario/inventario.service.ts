...
export class InventarioService {
  ...
  async getKpis(empresaId: number | undefined) {
    ...
    const valorTotal = productos.reduce(
      (acc, p) => acc + p.precioVenta * p.stock,
      0,
    );
  ...
  async predecirQuiebre(empresaId: number | undefined) {
    ...
    const porProducto = productos.map((p) => {
      ...
      const diasRestantes =
        promedioDiario > 0 ? Math.floor(p.stock / promedioDiario) : null;
    ...
    return porProducto.filter(
      (p) => p.diasEstimadosAntesDeAgotarStock !== null,
    );
  }
