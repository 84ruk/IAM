...
async function testSetupEmpresaWithRfc() {
  console.log('🧪 Probando setup de empresa con RFC...\n');
  
  try {
    // 1. Registrar un usuario nuevo
    console.log('1️⃣ Registrando usuario...');
    const registerResponse = await axios.post(`${BASE_URL}/auth/register`, {
      nombre: 'Usuario Test Setup RFC',
      email: `test-setup-rfc-${Date.now()}@example.com`,
      password: 'Test123!'
    });
    ...
    // 3. Configurar empresa con RFC
    console.log('\n3️⃣ Configurando empresa con RFC...');
    const setupResponse = await axios.post(`${BASE_URL}/auth/setup-empresa`, {
      nombreEmpresa: 'Empresa Test Con RFC',
      tipoIndustria: 'GENERICA',
      rfc: `TEST${Date.now()}`, // RFC único
      direccion: 'Dirección de prueba con RFC'
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    console.log('📋 Datos de la empresa:', {
      ...
      rfc: setupResponse.data.empresa.rfc
    });
...
// Ejecutar la prueba
testSetupEmpresaWithRfc().catch(console.error); 